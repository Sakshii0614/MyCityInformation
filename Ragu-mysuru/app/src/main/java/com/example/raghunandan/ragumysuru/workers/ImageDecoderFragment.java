...
package com.example.raghunandan.ragumysuru.workers;
...
public class ImageDecoderFragment extends Fragment
        implements LoaderManager.LoaderCallbacks<Bitmap> {

    //Constant used for logs and Fragment Tag
    private static final String LOG_TAG = ImageDecoderFragment.class.getSimpleName();
     ...
     * @return Instance of the Fragment {@link ImageDecoderFragment}
     */
    public static ImageDecoderFragment newInstance(FragmentManager fragmentManager, int tagId) {
        ...
        ImageDecoderFragment imageDecoderFragment =
                (ImageDecoderFragment) fragmentManager.findFragmentByTag(fragmentTagStr);
        if (imageDecoderFragment == null) {
            ...
            fragmentManager.beginTransaction().add(imageDecoderFragment, fragmentTagStr).commitAllowingStateLoss();
        }
    ...
    public void executeAndUpdate(ImageView imageView, @DrawableRes int imageResourceId, LoaderManager loaderManager) {
        ...
        ImageDecoder imageDecoder = getImageDecoder(imageResourceId, loaderManager);

        //Resetting the ImageView to the default Thumbnail Image for lazy loading
        mImageView.setImageResource(R.drawable.layer_all_default_picture);
        ...
        if (imageDecoder != null) {
            ...
            isNewImage = imageResourceId != imageDecoder.getImageResourceId();
        }
    ...
    /**
     * Method that returns the instance of the {@link ImageDecoder} for the
     ...
     * @return Instance of {@link ImageDecoder} if found; else {@code null}
     */
    @Nullable
    private ImageDecoder getImageDecoder(int loaderId, LoaderManager loaderManager) {
        //Getting the loader at the loaderId
        Loader<Bitmap> loader = loaderManager.getLoader(loaderId);
        if (loader != null && loader instanceof ImageDecoder) {
            //Returning the ImageDecoder instance
            return (ImageDecoder) loader;
        } else {
    ...
    public Loader<Bitmap> onCreateLoader(int id, @Nullable Bundle args) {
        //Returning an Instance of ImageDecoder to start the Image decoding
        return new ImageDecoder(mImageView.getContext(), mImageResourceId);
    }
    ...
    private void onDecodingFailure() {
        ...
        mImageView.setImageResource(R.drawable.layer_all_no_picture);
     ...
     * @return Instance of the this Fragment {@link ImageDecoderFragment} to chain method calls
     */
    public ImageDecoderFragment setOnFailureListener(OnFailureListener listener) {
     ...
     * @return Instance of the this Fragment {@link ImageDecoderFragment} to chain method calls
     */
    public ImageDecoderFragment setOnSuccessListener(OnSuccessListener listener) {
