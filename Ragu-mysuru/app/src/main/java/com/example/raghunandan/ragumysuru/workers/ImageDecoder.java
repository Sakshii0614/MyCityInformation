...
public class ImageDecoder extends AsyncTaskLoader<Bitmap> {
    ...
    /**
     * Constructor of the Loader {@link ImageDecoder}
     *
     ...
     */
    public ImageDecoder(@NonNull Context context, @DrawableRes int imageResourceId) {
        super(context);
    ...
    public Bitmap loadInBackground() {
        if (mImageResourceId > BitmapUtility.NO_RESOURCE_ID) {
            //When we have the Resource Id of the Image
            try {
                //Looking up for the Image in Memory Cache for the given Resource Id
                Bitmap cachedBitmap = BitmapImageCache.getBitmapFromCache(mImageResourceId);
                if (cachedBitmap != null) {
                    ...
                    Bitmap decodedBitmap = BitmapUtility.getOptimizedBitmapFromResource(getContext(), getContext().getResources(), mImageResourceId);
                    if (decodedBitmap != null) {
                        ...
                        BitmapImageCache.addBitmapToCache(mImageResourceId, decodedBitmap);
    ...
    private void releaseResources() {
        ...
        if (mDecodedBitmap != null) {
            mDecodedBitmap = null;
            mImageResourceId = BitmapUtility.NO_RESOURCE_ID;
        }
    }
